...
from lib.prune_all import (
    prune_wanda_outlier_structure_special,
    prune_wanda_outlier_structure,
    prune_sparsegpt_outlier,
    prune_wanda_outlier,
    prune_mag_outlier,
    prune_wanda,
    prune_magnitude,
    ...
    check_sparsity,
    find_layers,
    test_prune_wanda_outlier,
    test_prune_wanda_outlier_use_var,
    test_prune_wanda_outlier_use_evo,
)
from lib.autolayer import LayerEngine
...
def main():
    ...
    parser.add_argument("--layer_func", type=str, default="uniform")
    parser.add_argument("--shift", type=float, default=0.2)
    parser.add_argument("--step", type=float, default=0.5)

    parser.add_argument("--graph_string", type=str, default=None, help="graph_string")

    # args for evolution search
    parser.add_argument(
        "--iterations", default=2000, type=int, help="number of iteration for LSQ"
    )
    ...
    parser.add_argument(
        ...
        default=0.005,
        type=float,
        help="delta sparsity for mutation",
    )
    parser.add_argument(
        "--mutation_prob",
        default=0.2,
        type=float,
        help="mutation probability for mutation",
    )
    ...
    elif args.prune_method == "test_prune_wanda_outlier":
        test_prune_wanda_outlier(
            args,
            model,
            tokenizer,
    ...
    ############################ Ours ############################
    elif args.prune_method == "wanda_owl_var":
        lengine = LayerEngine.from_string("W:(ABS)-(VAR)-(9)")
        test_prune_wanda_outlier_use_var(
            args,
            model,
            tokenizer,
    ...
    elif args.prune_method == "wanda_owl_search":
        if args.graph_string is None:
            lengine = LayerEngine()
            graph_string = lengine.generate_random_graph()
    ...
    elif args.prune_method == "wanda_owl_evo":
        test_prune_wanda_outlier_use_evo(
            args,
            model,
            tokenizer,
        ...
        raise ValueError(f"invalid prune method {args.prune_method}")
